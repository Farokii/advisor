...
async def register(advisor: advisor_schema.AdvisorCreate, db: AsyncSession=Depends(get_db)):
    return await advisor_service.register(db,advisor)
#顾问-登录接口
@router.post("/login",response_model=dict)
async def login(advisor: advisor_schema.AdvisorLogin, db: AsyncSession=Depends(get_db)):
    return await advisor_service.login(db,advisor)
#顾问-信息修改
...
async def update_profile(
    ...
    return await advisor_service.update_profile(db,advisor,current_advisor_id)
#顾问-个人主页
@router.get("/profile",response_model=advisor_schema.ProfileResponse)
async def profile(db: AsyncSession = Depends(get_db), current_advisor_id: int = Depends(dependencies.get_current_user_id)):
    return await advisor_service.profile(db,current_advisor_id)
#顾问-接单状态更新
...
async def work_status(
    ...
    return await advisor_service.update_work_status(db,current_advisor_id,advisor)
#顾问-服务状态更新
...
async def service_status(
    ...
    return await advisor_service.update_service_status(db,current_advisor_id,advisor)
#顾问-价格修改
...
async def price(
    ...
    return await advisor_service.update_price(db,current_advisor_id,advisor)
#顾问-订单列表
@router.get("/order-list",response_model = List[order_schema.OrderListResponse])
async def order_list(db: AsyncSession = Depends(get_db), current_advisor_id: int = Depends(dependencies.get_current_user_id)):
    advisor_order_list = await advisor_service.order_list(db,current_advisor_id)
...
async def order_details(order_id: int, db: AsyncSession = Depends(get_db), current_advisor_id: int = Depends(dependencies.get_current_user_id)):
    advisor_order_details = await advisor_service.order_details(db, current_advisor_id, order_id)
...
async def complete_order(order_id: int,
                         ...
                         db: AsyncSession = Depends(get_db),
                         current_advisor_id: int = Depends(dependencies.get_current_user_id)):
    return await advisor_service.complete_order(db, current_advisor_id, order_id, reply)
# 顾问-流水列表
...
async def coin_trans(advisor_id: int = Depends(dependencies.get_current_user_id)):
    return await advisor_service.coin_trans(advisor_id)

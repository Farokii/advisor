...
#1.顾问端-注册
async def register(db: AsyncSession, advisor: advisor_schema.AdvisorCreate):
    #检查手机号是否已注册
    db_advisor = await advisor_crud.get_advisor_by_phone(db, phone=advisor.phone_number)
    if db_advisor:
        raise HTTPException(status_code=400, detail="Phone already registered")
    return await advisor_crud.create_advisor(db,advisor)
#2.顾问端-登录
async def login(db: AsyncSession, advisor: advisor_schema.AdvisorLogin):
    # 通过手机号验证登录用户是否存在
    db_advisor = await advisor_crud.get_advisor_by_phone(db, advisor.phone_number)
    if not db_advisor:
        raise HTTPException(
...
async def update_profile(db: AsyncSession, advisor: advisor_schema.AdvisorUpdateProfile, advisor_id: int):
    updated_advisor = await advisor_crud.update_profile(db, advisor, advisor_id)
...
async def profile(db: AsyncSession, advisor_id: int):
    advisor_profile = await advisor_crud.profile(db, advisor_id)
    if advisor_profile is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Failed to view profile",
        )
    return await advisor_crud.profile(db, advisor_id)
#5.顾问端-接单状态更新
async def update_work_status(db: AsyncSession, advisor_id: int, advisor: advisor_schema.UpdateWorkStatus):
    updated_advisor = await advisor_crud.update_work_status(db, advisor_id, advisor)
    if updated_advisor is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def update_service_status(db: AsyncSession, advisor_id: int, advisor: advisor_schema.UpdateServiceStatus):
    updated_advisor = await advisor_crud.update_service_status(db, advisor_id, advisor)
    if updated_advisor is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def update_price(db: AsyncSession, advisor_id: int, advisor: advisor_schema.UpdatePrice):
    updated_advisor = await advisor_crud.update_price(db, advisor_id, advisor)
    if updated_advisor is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    return updated_advisor
# 8.顾问端-查看订单列表
async def order_list(db: AsyncSession, advisor_id: int):
    return await order_crud.advisor_order_list(db, advisor_id)
# 9.顾问端-查看订单详情
async def order_details(db: AsyncSession, advisor_id: int, order_id: int):
...
# 10.顾问端-回复订单
async def complete_order(db: AsyncSession, advisor_id: int, order_id: int, reply: advisor_schema.Reply):
    order = await order_crud.get_order_by_id(db, order_id)
    if order is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    return await order_crud.complete_order(db, order_id, advisor_id,reply)

#顾问端-流水列表
async def coin_trans(advisor_id: int):
    logs = await get_coin_trans("advisor", advisor_id)
    return [
